...
- **`ic_download_document`** is the only write/IO tool (writes to disk). It does pre-flight checks for directory destinations, missing parent dir, and existing files (requires `overwrite: true`). Supports absolute URLs as well as relative `/campus/...` paths, but the resolved URL must be **https on the district's own origin** — anything else is refused with `DocumentOriginNotAllowed` before any request, because the fetch carries the parent's IC session cookies and `documentId` is model-controlled (fleet-audit#144).
