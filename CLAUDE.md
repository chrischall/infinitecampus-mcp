...
- `src/client.ts` — `ICClient` with per-district session pool, lazy login, 401 retry, download method
...
- Parent portal uses Spring Security session cookies (`JSESSIONID`); login flow is `GET /campus/portal/parents/<district>.jsp` to get an initial cookie, then `POST /campus/verify.jsp?nonBrowser=true&...`
...
- Multi-district: every tool takes `district` as its first arg; `ic_list_districts` returns valid names. Sessions are per-district and lazy-loaded; querying district A never touches district B's session
